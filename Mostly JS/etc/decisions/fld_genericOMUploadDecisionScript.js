...
function isWorkBundleReady(/*WorkBundle*/ workBundle) {
  try {
    ...
    // Checks for MatId & TrackTypeLink
    var rtnObj = fmh.runChecksOnFileForTTL(CHECK_BASENAME_AS_WELL);
    ...
    if (bActionWorkbundle){
      ...
      assignJobDescriptionToWorkBundle(workBundle, rtnObj.matId);
    }
  } catch (e) {
    logger.info(e);
    return false;
...
function assignJobDescriptionToWorkBundle(workBundle, kMatId) {
  ...
  workBundle.jobDescription.setProperty("FolderMonitorMatId", kMatId);
  logger.info("\tSetting Job Description FolderMonitorMatId Property to [" + kMatId + "]");
}
