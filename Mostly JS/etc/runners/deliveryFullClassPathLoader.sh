...
DELIVERY_SCRIPT=/opt/evertz/mediator/etc/runners/run_contentDistributionDelivery.js

echo -e "\nIn deliveryFullClassPathLoader.sh Running ${DELIVERY_SCRIPT} \nArguments  are ${@}"

java -Dfile.encoding=utf-8 -cp "$RHINO_CLASSPATH" "$SHELL" "$DELIVERY_SCRIPT" "$@"
