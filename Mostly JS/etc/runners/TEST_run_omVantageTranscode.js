...
load("/usr/local/pharos/etc/scripts/test/TEST_nbcgmo_vantage_fun.js");
load("/usr/local/pharos/etc/scripts/test/TEST_nbcgmo_fun.js");
load("/usr/local/pharos/etc/scripts/vantage_lookup.js");
...
try{
	...
	sourceObj.stagingmediamount = lookup.media[stagingMedia].mount;
	...
	*/
	...
	if(!transcodeEnabled){
		...
		//set the audio and video media based on what is stored in the settigns file
		if(isStraightToArchive){
			print("\nThis is straight to Archive workflow - Resetting Store Media with T2 media")
			sourceObj.metadata.defaultStorageMedia = "T2_"+sourceObj.metadata.defaultStorageMedia;
		}
	...
	var vantageWFGMO = "GMO_INGEST_MOVE_AND_AUDIO_EXTRACTION";
	...
	sourceObj.timecodedata = gmoNBCFunc.getMovTimeCodes(sourceObj.videomediapathandfile);
	...
	track = materialXML..Track.(MediaName.toString()=='OM_STAGING');
	...
	if(captionExtraction){
		...
		jobDashboard.updateStatusAndProgress("Tansitioning to " + qcTrigger,95);
