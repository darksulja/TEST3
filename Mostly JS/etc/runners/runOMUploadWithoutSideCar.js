...
var validateAudioStreams = function(sourceObj) {
	...
	var xmlAudioChannels = sourceObj.totalAudioChannels;
	...
	if(xmlAudioChannels > audioChannels){
		throw new Error("More Track types defines in xml than in the file");
		// We cannot have more track types in mediator than we have on the file 
	}else if(xmlAudioChannels < audioChannels){
		print("\nWe have more audio tracks in he file than specified in the audio profile.");
		gmoNBCFunc.addComment(sourceObj.matid, "Upload", "File has more audio tracks than what has been specified in the sidecar xml");
...
var updateForcedNarrativeLanguageTag = function(_obj) {
	...
	var fnCode 		= _sidecarxml.Slate.ID_List.ID.(Type.toString()=="Burn in FN").Value.toString();
    var fnLanguage 	= gmoNBCFunc.isVarUsable(fnCode) ? gmoNBCFunc.getTagByTagTypeAndValue("Language Code Lookup",fnCode.toLowerCase()).Description.toString() : "";

	var subCode		= _sidecarxml.Slate.ID_List.ID.(Type.toString()=="Burn In Subtitle").Value.toString();
	var subLanguage = gmoNBCFunc.isVarUsable(subCode) ? gmoNBCFunc.getTagByTagTypeAndValue("Language Code Lookup",subCode.toLowerCase()).Description.toString() : "";
	...
	var isSave = false;					
    if (gmoNBCFunc.isVarUsable(fnLanguage)){
	...
	if (gmoNBCFunc.isVarUsable(subLanguage)) {
	...
	if (isSave) {
		try {
			...
			throw new Error("updateForcedNarrativeLanguageTag(): Issue updating Forced Narrative/Subtitle Burn in information. Please check that the languages are configured - otherwise escalate to Mediator Support");
		}
	} else {
		return true;
	}
...
try { 
	...
	//Extract Language Master Values
	var lmLangArray = [];
	var primaryLanguage = sourceObj.parsedxml.Slate.ID_List.ID.(Type.toString()=="Primary Language").Value.toString();
	gmoNBCFunc.isVarUsable(primaryLanguage) ? lmLangArray.push(primaryLanguage) : lmLangArray.push("");

	var secondaryLanguage = sourceObj.parsedxml.Slate.ID_List.ID.(Type.toString()=="Secondary Language").Value.toString();
	gmoNBCFunc.isVarUsable(secondaryLanguage) ? lmLangArray.push(secondaryLanguage) : lmLangArray.push("");

	var tertiaryLanguage = sourceObj.parsedxml.Slate.ID_List.ID.(Type.toString()=="Tertiary Language").Value.toString();
	gmoNBCFunc.isVarUsable(tertiaryLanguage) ? lmLangArray.push(tertiaryLanguage) : lmLangArray.push("");

	var fourthLanguage = sourceObj.parsedxml.Slate.ID_List.ID.(Type.toString()=="Fourth Language").Value.toString();
	gmoNBCFunc.isVarUsable(fourthLanguage) ? lmLangArray.push(fourthLanguage) : lmLangArray.push("");

	print(lmLangArray);
	var audioProfileName = sourceObj.parsedxml.Slate.AudioConfig.toString();
	print(audioProfileName);
	...
	sourceObj.xmltracktypes = profileTrackTypes;
	print("\n\nXML Track Types ["+sourceObj.xmltracktypes+"] \nAttempting to Map Xml Track Types to Mediator Track Types");

	sourceObj.totalAudioChannels = audioProfile.getTotalNumberOfChannels();
	print("\n\nAudio Profile specified a total of ["+sourceObj.totalAudioChannels+"] audio channels.");

	var handlePotentialDuplicates = unmuddlePossibleDuplicateAudios(sourceObj.xmltracktypes);
