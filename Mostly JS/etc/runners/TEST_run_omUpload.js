...
load("/usr/local/pharos/etc/scripts/test/TEST_nbcgmo_fun.js");
load("/usr/local/pharos/etc/scripts/nbcgmo_settings.js");
load("/usr/local/pharos/etc/scripts/lookup.js");
...
var validateAudioStreams = function(sourceObj) {
	...
	for (var i=0; i<sourceObj.mediainfoxml.track.(@type.toString()==="Audio").length(); i++) {
		...
		if (parseInt(audioTrack.Bit_rate[0].toString()) != audioSettings.BitRate) throw new Error ("Bit Rate Mismatch Expected ["+audioSettings.BitRate+"] Actual ["+audioTrack.Bit_rate[0].toString()+"]\n"+audioTrack);
...
var buildSourceObj = function() {
	...
	if (videoState === states.orderPlaced) {
...
var saveMetaData = function (sourceObj) {
	...
	matSaveXml.FullTextList.appendChild(gmoNBCFunc.createFullTextNode(minfoFullTextType,sourceObj.mediainfoxml.toString()));
...
try { 
	...
	if (sourceObj.matxml.Track.(MediaName.toString()===sourceObj.stagingmedia && parseInt(DeleteMark) === 0 ).length() !== 0) {
	...
	if (sourceObj.extension === extensions.mov) { 
		sourceObj.timecodedata = gmoNBCFunc.getMovTimeCodes(sourceObj.vidpath);
	...
	if (cleanUpBools.deletetrack) { 
		print("\nError CleanUp: Deleting Track for Material [" + matId + "] on Media [" + stagingMedia + "]");
		gmoNBCFunc.materialTrackDelete(matId,stagingMedia);
	}
