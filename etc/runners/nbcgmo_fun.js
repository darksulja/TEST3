...
//		}
	...
	*/
    ...
    transitionTrackTypes : function(matId, requirement, tracktypes) {
		...
		var trackTypeLinks = new XMLList(), requirementParameter, materialParameter, transitionXml;

		//If they pass in one item or a string comma seperated list, make it into an array
		if (typeof(tracktypes) === "string") tracktypes = tracktypes.split(",");

		for each (var tt in tracktypes) {
			trackTypeLinks += <TrackTypeLink>
								<TrackTypeName>{tt}</TrackTypeName>
							  </TrackTypeLink>;
		}
	...
	*/
	...
	getMediaTrackFilePath : function(matId, mediaName){
		...
		result = wscall(cmd)..Output.Media;
		var dirNameFactory = result.DirectoryNameFactoryName.toString();
		if (dirNameFactory == "FlatListDirectoryNameFactory") {
			return "";
		} else if (dirNameFactory == "MaterialDirectoryNameFactory") {
			return "/" + matId + ".dir/";
			...
			// Assume FlatListDirectoryNameFactory if not assigned.
			return "";
		}
	},
	...
	usefulFileObj	:	function(source_file) {
		...
		this.exists = function() {
			print("\nusefulFileObj.exists(): Checking if [" + this.unix_file + "] exists");
	...
	*/
	saveNote : function(domainType,domainKey,noteType,noteClass,importance,content) {
		var xml = <PharosCs>
				...
				print('print XML I am trying to send')
				print(xml)
		return wscall(xml);
	},
	...
	*/
	createTrackDefPositionMap : function(matId, mediaName){
		...
		for each (var trackDef in materialXml.Track.(MediaName == mediaName).TrackDefinition){
            print('trackDef is: [' + gmoNBCFunc.printObj(trackDef) + "]");
	...
	materialHelper	:	function(matId) {
		...
		*/
		...
		this.getDuration = function () {
			return this.materialXml..Duration.toString();
		}	
		...
		this.isUHD = function() {
            return this.materialXml..Tag.(TagType == "UHD/HDR").Value.toString().indexOf("UHD") > -1 ? true : false;
        };
	...
	*/
	...
	getSystemStreamXml : function () {
		return wscall(
			<PharosCs>
				<CommandList>
					<Command subsystem="playtime" method="getStreams">
						<ParameterList>
							<Parameter name="allStreams">
								<Value>
									<Boolean>true</Boolean>
								</Value>
							</Parameter>
    ...
    // Removing trackTypeGet - see getTrackType in profileHelper instead
    // This has been here since August 2017 but no one was using it

	getClassOfTrackType : function(trackTypeName) {
		var trackTypeXml = this.trackTypeGet(trackTypeName);
		return trackTypeXml..ClassName.toString();
	},
